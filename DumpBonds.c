...
void DumpBonds(){
  ...
  fprintf(fpbond, "ITEM: ENTRIES BondID, BondType, atom1 atom2 kb ro BondEnergy BondLength\n");

  for(n=1; n<=nBond; n++)
   fprintf(fpbond, "%d %d %d %d %lf %lf %lf %lf\n", BondID[n], BondType[n], atom1[n], atom2[n], kb[n], ro[n], BondEnergy[n], BondLength[n]);
  }
