...
void Trajectory(){
  ...
  fprintf(fpxyz, "ITEM: ATOMS id type radius x y vx vy fx fy fdx fdy\n");
  for(n=1; n<=nAtom; n++)
   fprintf(fpxyz, "%d\t %d\t %0.2lf\t %0.16lf\t %0.16lf\t %0.16lf\t %0.16lf\t %0.16lf\t %0.16lf\t %0.16lf\t %0.16lf\n", 
   atomID[n], atomType[n], atomRadius[n], rx[n], ry[n], vx[n], vy[n], ax[n], ay[n], discDragx[n], discDragy[n]);
  }
